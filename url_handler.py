...
class URL_Handler:
    ...
    static_files = {
        ...
        "flashbutton.png": "image/png",
        "twinklebutton.png": "image/png",
    ...
    def change_led (self, request):
        ...
        # Look for flash request
        m = re.search ('light=(\d)&action=flash', request)
        if m != None:
            led_selected = int(m.group(1))-1
            # check valid number
            if (led_selected >=0 and led_selected <= 2) :
                return (led_selected, "flash")
        # Look for twinkle request
        m = re.search ('light=(\d)&action=twinkle', request)
        if m != None:
            led_selected = int(m.group(1))-1
            # check valid number
            if (led_selected >=0 and led_selected <= 2) :
                return (led_selected, "twinkle")
        return None
